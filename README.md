...
- **Deck Builder** (Ctrl+L). Build decks for any format: Standard, Pioneer, Modern, Legacy, Vintage, Pauper, Commander, Oathbreaker, Brawl and the other formats Scryfall tracks, plus binders and wishlists. Three panels: the selected card's details (rules text, legality, how many you own, −1 / +1), the deck grouped by section, and a card list shown as a grid of card images with the number you own under each: *My Cards* (your collection) or *Explore* (every card in Magic), with search, type, color and "legal for this deck" filters and sorting. *View → Lightweight* swaps the images for a plain table that downloads no images, for slower computers or connections. Choosing a different printing opens a picker showing every printing with its image, set, rarity, and each finish's price and how many you own. Every deck is checked against its format as you build: deck and sideboard size, copy limits (basic lands and "any number" cards aside), banned and restricted cards, and commander eligibility and color identity. Cards you don't own are marked, and each list shows what it's worth and what the missing cards would cost. Deck lists import from Arena, Moxfield and most other tools, and export as standard text. The card database (about 80 MB from Scryfall) downloads once and updates weekly.
